...
Hal_console_usart_init_mode(void)
{
    ...
    *US_MR = 0;                                                      // Clear
    *US_MR |= USART_MR_MODE_NORMAL << USART_MR_MODE_OFFSET;          // UART_MODE NORMAL
    ...
    *US_MR |= USART_MR_CHMODE_NORMAL << UART_MR_CHMODE_OFFSET;      // CHMODE Normal mode
...
Hal_console_usart_init_baudrate()
{
    ...
    uint64_t fineDiv = 8uLL
                       * (((uint64_t)DEFAULT_PERIPH_CLOCK * 1000uLL / (16uLL * USART_BAUD_RATE))
...
Hal_console_usart_init_tx_enable()
{
    ...
    *US_CR = USART_CR_TXEN_ENABLE << UART_CR_TXEN_OFFSET; // Enable the transmitter
}
