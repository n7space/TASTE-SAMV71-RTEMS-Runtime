...
{
  const uint32_t stack_size = (uint32_t)&SECTION_STACK_END - deathReport->stackTracePointer;
  deathReport->stackTraceLength = minUInt32(stackSize, DEATH_REPORT_STACK_TRACE_SIZE);
  const uint32_t* const stack = (const uint32_t*)deathReport->stackTracePointer;
  for (uint32_t i = 0; i < deathReport->stackTraceLength; i++)
    deathReport->stackTrace[i] = stack[i];
}

void __attribute__((noinline)) Fault_HandlerTail(void)
{
  DeathReportWriter_DeathReport* const death_report = boot_report + DEATH_REPORT_OFFSET;
  save_stack(deathReport);
  (void)memcpy(&deathReport->selfTestsResults,
    &BOOT_REPORT->selfTestsResults,
    sizeof(BootReportWriter_SelfTestsResults));
  deathReport->padding = 0u;
  deathReport->wasSeen = false;
  BootReportWriter_updateDeathReportCrc(deathReport);
