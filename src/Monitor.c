...
static void UpdateActivationLog(const uint32_t index,
				...
				const enum Monitor_EntryType entry_type)
{
	...
	activation_log_buffer[latest_activation_entry_index].interface =
		interface;
	activation_log_buffer[latest_activation_entry_index].entry_type =
		entry_type;
	activation_log_buffer[latest_activation_entry_index].timestamp =
		timestamp;
}

static bool
HandleActivationLogCyclicBuffer(const enum interfaces_enum interface,
				const enum Monitor_EntryType entry_Type)
{
	...
	if (latest_activation_entry_index == 0 && size_of_activation_log == 0) {
		UpdateActivationLog(latest_activation_entry_index, interface,
				    entry_Type);
		size_of_activation_log++;
	} else if ((latest_activation_entry_index < RT_EXEC_LOG_SIZE - 1) &&
		   (size_of_activation_log < RT_EXEC_LOG_SIZE)) {
...
{
	return HandleActivationLogCyclicBuffer(interface,
					       Monitor_EntryType_activation);
}

bool Monitor_IndicateInterfaceDeactivated(const enum interfaces_enum interface)
{
	return HandleActivationLogCyclicBuffer(interface,
					       Monitor_EntryType_deactivation);
}
